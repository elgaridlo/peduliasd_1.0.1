...
exports.createArticle = catchAsync(async (req, res, next) => {
  const insert = await knex('articles')
    .insert({
      ...req.body,
      urlTitle: req.body.title.toLowerCase().replace(/[^a-z/0-9]+/g, ' ').replaceAll(' ', '-')
    })
...
exports.getArticleById = catchAsync(async (req, res, next) => {
  const article = await knex('articles').select().where({ urlTitle: req.params.id })
...
exports.getAllArticle = catchAsync(async (req, res, next) => {
  ...
  const articles = await knex.select('*')
    .from('articles').orderBy('id', 'desc').limit(size).offset(skip)
...
exports.updateArticle = catchAsync(async (req, res, next) => {
  ...
  await knex('articles').where('id', req.params.id)
    .update({
      ...req.body, urlTitle: req.body.title.toLowerCase().replace(/[^a-z/0-9]+/g, ' ').replaceAll(' ', '-')
    })
