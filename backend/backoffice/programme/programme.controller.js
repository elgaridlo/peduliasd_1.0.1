...
exports.createProgrammes = catchAsync(async (req, res) => {
  const insert = await knex('programmes').insert({
    ...req.body,
  });
...
exports.getProgrammesById = catchAsync(async (req, res, next) => {
  const programmes = await knex('programmes')
    .select()
